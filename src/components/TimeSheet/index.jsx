...
const TimeSheet = () => {
  ...
  useEffect(async () => {
    ...
    setTimesheetData(data?.content);
    setCurrentRow(timesheetData[0])
    setTotalPages(data?.totalPages);
  }, [isReload, currentProject, page, pageSize]);
  ...
  return (<div className={`col-10 ${styles.timesheet}`}>
    <Box sx={{ flexGrow: 1 }}>
          ...
          <div className={styles.toolbar_buttons}>
            ...
            >
              ...
              {projectList && (projectList.map((item) => {
                return <option key={item?.id} value={item?.projectName}>{item?.projectName}</option>
              }))}
    ...
    {timesheetData && timesheetData?.message !== 'No Items Found' && (<TableContainer component={Paper} className={styles.table_container} >
    ...
    {(!timesheetData || timesheetData?.message === 'No Items Found') && (<>
