...
const Reports = () => {
  ...
  const [handleCsv, setHandleCsv] = useState(false);
  ...
  return (
    <div style={{ width: '80%' }} className={`col-10 ${styles.reports}`}>
      <Box sx={{ flexGrow: 1 }}>
        <select
          value={currentReport}
        ...
        <AppBar position="static">
          <Toolbar variant="dense" className='toolbar-align'>
            ...
            <div direction="row" className={styles.toolbar_buttons} >
              <Button className={styles.filter_entry}><span>Click to Filter</span> <span><FilterListIcon onClick={() => { setOpen(!open) }} /></span></Button>
        ...
        <Dialog open={open}  >
          <DialogContent>
            <FormControl className={styles.form_wrapper}>
              <label>From</label>
              <Stack direction='row'> <select value={fromMonth} onChange={(e) => { handleChangeFromMonth(e) }}> {monthData.map((monthObj) => {
                return <option value={monthObj.key}>{monthObj.value}</option>
              })}
              </select>
          ...
          <DialogActions>
            <Button onClick={(e) => { setOpen(!open) }}>Cancel</Button>
            <Button onClick={handleApplyFilters}>Apply Filters</Button>
          </DialogActions>
        </Dialog>
      )}
      {currentReport === 'Resource' && (<ResourceReport fromMonth={fromMonth} fromYear={fromYear} toMonth={toMonth} toYear={toYear} setFromMonth={setFromMonth} setToMonth={setToMonth} setToYear={setToYear} setFromYear={setFromYear} isReload={isReload} setIsReload={setIsReload} open={open} setOpen={setOpen} handleCsv={handleCsv} setHandleCsv={setHandleCsv} />)}
    </div>)
